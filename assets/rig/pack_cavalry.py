...
for faction in ['cavalry']:
 ...
 for row,direction in enumerate(dirs):
  ...
  for frame in range(count):
   ...
   image=Image.open(ROOT/'rig/cavalry-frames'/name).convert('RGBA').resize((size,size),Image.Resampling.NEAREST)
   ...
   frames.append({'direction':direction,'frame':frame,'rect':[frame*size,row*size,size,size],
    'alpha_bounds':list(bounds),'sha256':hashlib.sha256((ROOT/'rig/cavalry-frames'/name).read_bytes()).hexdigest()})
 ...
 for index,direction in enumerate(dirs):idle.alpha_composite(Image.open(ROOT/f'rig/cavalry-frames/{faction}-idle-{direction}.png').convert('RGBA').resize((size,size),Image.Resampling.NEAREST),(size*index,0))
...
(out/'cavalry-animation.json').write_text(json.dumps(manifest,indent=2)+'\n')
print('Packed cavalry eight-direction walk atlas, idle atlas and animated previews.')

import shutil
public=ROOT.parent/'web/public/art'
for file in out.glob('cavalry-*'):
 if 'atlas' in file.name or 'preview' in file.name or file.name=='cavalry-animation.json':shutil.copy2(file,public/file.name)
