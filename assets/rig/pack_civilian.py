...
for action in ['idle','walk']:
 ...
 for row,direction in enumerate(dirs):
  for frame in range(1 if action=='idle' else 8):
   im=Image.open(ROOT/f'rig/civilian-frames/civilian-{action}-{direction}{"" if action=="idle" else "-"+str(frame)}.png').convert('RGBA').resize((size,size),Image.Resampling.NEAREST);bounds=im.getchannel('A').getbbox();assert bounds and bounds[0]>0 and bounds[1]>0 and bounds[2]<192 and bounds[3]<192
