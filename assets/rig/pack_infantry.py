...
for faction in ['granadero','royalist']:
 ...
 for row,direction in enumerate(dirs):
  ...
  for frame in range(count):
   ...
   image=Image.open(ROOT/'rig/frames'/name).convert('RGBA')
 ...
 for index,direction in enumerate(dirs):idle.alpha_composite(Image.open(ROOT/f'rig/frames/{faction}-idle-{direction}.png').convert('RGBA'),(size*index,0))
