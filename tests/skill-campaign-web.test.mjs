...
test('riding practice earned by moving persists without double applying its skill bonus',async()=>{
 ...
 let s=initialCampaign();s=dispatchCampaign(s,{type:'horseAction',order:{type:'acquire',sex:'mare'}});assert.equal(s.lastError,null);s=dispatchCampaign(s,{type:'horseAction',order:{type:'assign',horseId:'horse-1',operativeId:3}});assert.equal(s.lastError,null);s.operativeState[3].skillPractice={ridingSkill:39};s=dispatchCampaign(s,{type:'visitSector'});let battle=enterSector(s.pendingBattle);const u=battle.units.find(u=>u.id==='3');
