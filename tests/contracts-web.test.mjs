...
test('elite renewals never stack beyond one day and dead recruits cannot be hired',()=>{
 let s=order(initialCampaign(),create);const elites=rosterFor(s).filter(o=>o.tier==='elite');assert.ok(elites.length>=2);
 const id=elites[0].id;s=order(s,{type:'recruitCivic',id,term:'day'});const cash=s.resources.treasury;
 let rejected=dispatchCampaign(s,{type:'renewContract',id,term:'day'});assert.ok(rejected.lastError);assert.equal(rejected.resources.treasury,cash);
 s=order(s,{type:'wait',hours:6});s=order(s,{type:'renewContract',id,term:'day'});assert.equal(s.contracts[id].expiresAt-s.hour,24);
 rejected=dispatchCampaign(s,{type:'renewContract',id,term:'day'});assert.ok(rejected.lastError);
