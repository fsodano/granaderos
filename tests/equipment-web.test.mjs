...
const order=(s,a)=>{let n=dispatch(marchToFront(s,a),a);assert.equal(n.lastError,null,n.lastError);if(a.type==='purchaseEquipment'&&n.equipmentShipments?.length){assert.equal(n.resources.treasury,s.resources.treasury-EQUIPMENT_CATALOG.find(i=>i.item===a.item).price*(a.quantity??1));n=dispatch(n,{type:'wait',hours:n.equipmentShipments[0].due-n.hour});assert.equal(n.lastError,null);}return n;};
test('all9 firearms and5blades are purchasable canonical equipment',()=>{
 ...
 for(const item of EQUIPMENT_CATALOG.filter(i=>i.category!=='artillery')){let s=order(initialCampaign(),{type:'purchaseEquipment',item:item.item});assert.equal(s.armory[item.item],1);s=order(s,{type:'equip',operativeId:item.category==='blade'&&item.id===1811?4:3,slot:item.category==='blade'?'blade':'weapon',itemId:item.id});assert.equal(s.armory[item.item],0);assert.equal(rosterFor(s).find(o=>o.id===(item.category==='blade'&&item.id===1811?4:3))[item.category==='blade'?'blade':'weapon'],item.id);}
});
