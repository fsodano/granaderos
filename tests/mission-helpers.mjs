...
export function attendYatasto(s){
 ...
 for(const npcId of ['yatasto-belgrano','yatasto-san-martin','yatasto-san-martin']){const npc=b.npcs.find(n=>n.id===npcId);const p=getReachable(b,actor.id).find(p=>Math.abs(p.x-npc.x)+Math.abs(p.y-npc.y)===1);if(!p)throw Error('Conference interlocutor unreachable');if(p.cost)b=actBattle(b,{type:'move',unitId:actor.id,x:p.x,y:p.y});if(b.lastError)throw Error(b.lastError);s=step(s,{type:'talkNPC',npcId,approach:'mission',unitId:Number(actor.id),sectorState:b});}
