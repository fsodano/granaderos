...
test('trained local militia become real allied soldiers and retain finite ammo across visits',()=>{
 let s=trained();const stock=s.resources.treasury;s=step(s,{type:'visitSector'});assert.equal(s.pendingBattle.garrison.length,3);let b=enterSector(s.pendingBattle);const militia=b.units.filter(u=>u.militia);assert.equal(militia.length,3);assert.equal(new Set(b.units.map(u=>`${u.x},${u.y}`)).size,b.units.length);const ids=militia.map(u=>u.id);s=leave(s,b);assert.equal(s.resources.treasury,stock);s=restoreCampaign(serializeCampaign(s));s=step(s,{type:'visitSector'});b=enterSector(s.pendingBattle,s.sectorStates.retiro);assert.deepEqual(b.units.filter(u=>u.militia).map(u=>u.id),ids);s=leave(s,b);assert.equal(s.resources.treasury,stock);s=step(s,{type:'travel',sector:'buenos_aires'});s=step(s,{type:'visitSector'});assert.equal(s.pendingBattle.garrison.length,0);
});
