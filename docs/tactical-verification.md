...
## Authored opening campaign playthrough

Run `node --test tests/opening-playthrough.test.mjs`. The test starts `initialCampaign(7)`, pays for the academy, travels to Buenos Aires, and issues a San Nicolás attack. Both encounters use `buildSectorMap`, exactly as the browser does. A deterministic greedy controller submits ordinary healing, melee, shooting, reload, charge, movement and end-turn orders; unsuccessful candidate orders are discarded through the public immutable action API. It never assigns victory, health, ammunition or positions directly.

San Nicolás ends in victory on turn 2 after 8 accepted actions. Cabral dies; Dorrego has 67 HP and Paroissien 72. The actual survivor report is returned to campaign, followed by legal travel to San Nicolás and 120 strategic hours of recovery. San Lorenzo ends in victory on turn 3 after 18 accepted actions: Dorrego has 84 HP; Paroissien survives with 24 HP and has routed. The second actual result unlocks campaign phase 2. Cabral remains dead and the remaining squad contains only Dorrego and Paroissien. Both battles consume issued ammunition and replay identically from their original requests.

This is a reproducible successful route, not a claim that the opening is forgiving: the same simple controller lost with seed 1812 and several other seeds. Seed 7 was selected after exploring deterministic seeds 1–7. The evidence establishes that both authored encounters and campaign transitions are playable without fabricated results; it does not establish balanced difficulty or intelligent player strategy. No gameplay code change was needed for this verification. Full Node suite: 156 tests pass.

## Use-based practice

`game/skill-training.js` tracks finite deterministic runtime practice independently of authored stats. Forty practice points earn one attribute point, capped at ten earned points per skill and an absolute attribute ceiling of 100. An actual discharged shot grants two marksmanship points (including misses, excluding failed ignition); successful finite healing grants three medical points; changing a consumed flint grants three mechanical points; successful repriming consumes powder and grants one mechanical point. Invalid actions grant nothing. Crouching/prone movement grants one agility and stealth practice point only on a previously unpractised tile while within twelve tiles of a living hostile that cannot see the mover. Repeated movement over the same tactical tile cannot farm practice. Earned stealth slightly reduces detection range while sneaking, up to one tile; illumination still reveals targets. Legacy records need no training fields.

The dedicated skill tests exercise real public actions, invalid repetitions, finite supplies, capped growth, and NPC movement occupancy. Training counters and bonuses are validated at the tactical snapshot boundary; campaign persistence is coordinated with the campaign module owner.

## Custom character advancement

The player's custom Granadero now uses the same level thresholds as fictional recruits, preserving manually allocated starting attributes as the immutable basis. `tests/custom-progression.test.mjs` starts a saved veteran fixture at 95 XP and one practice point short of improvement, performs an actual flint repair, and reports a retreat for the existing 10 XP award. It verifies level two, the sum of level and practice bonuses exactly once, and portrait/personality/night skill preservation after campaign serialization and sector re-entry. This exercises two distinct requirements: accepting accrued XP in roster reconstruction and awarding battle XP to the player's own character.

## Shared campaign clock

`syncBattleTime(campaign, battle)` in `game/time.js` is the paired browser bridge. Successful tactical actions accumulate integer seconds. Exploration uses actual traversed tiles (run 1 s, walk/crouch 3 s, prone 5 s), AP-scaled action durations, and a ten-minute rest. Combat charges six seconds once per round, on its first accepted action or on an otherwise idle end-turn, covering victories within that round. Failed actions and navigation consume no time. Campaign hours remain integers with `secondOfHour` remainder; each deployment records its acknowledged elapsed seconds and repeated synchronization is idempotent. Save decoding rejects crossed deployment IDs or unequal clocks and supplies legacy clock defaults.

Hourly economy and remote contracts continue while deployed. Expired deployed contracts wait for the survivor report; deployed troops cannot heal strategically at midnight. Horses retain tactical stamina and cannot disappear when a lease expires in battle; their breeding calendar still advances. Raids aimed at the occupied sector queue until the result and garrison losses are reconciled. Torch lifetimes use seconds, including strategic absence, and night visibility changes when the clock crosses dawn/dusk.

`tests/time-bridge.test.mjs` exercises actual movement/rest, fractional dawn, save/resume idempotency, deployed versus remote contract expiry, midnight wounds, raid deferral, mounted stamina, torch absence aging, and stale-clock rejection. Seven integration tests pass. The browser calls the bridge for every tactical update and saves the returned campaign/battle pair.
