...
### Authored historical mission scenes

`missions.js` registers Yatasto and San Lorenzo. Yatasto is a separate walkable posta scene anchored to Tucumán for strategic travel, with its own `sceneStates.yatasto` snapshot; it does not overwrite Tucumán's ordinary map. `visitMission {mission:'yatasto'}` requires physical presence in Tucumán after San Lorenzo. Adjacent `talkNPC` with `approach:'mission'` examines Belgrano's reports, discusses San Martín's continental alternative, then confirms the Güemes pact and supplied Salta route. `finishMission` validates all stages and reconciles the tactical visit before advancing toward Cuyo. Leaving early preserves progress. Dialogue is dramatized; the scene placement is a strategic-map abstraction.

San Lorenzo deploys a temporary allied San Martín through `missionAllies`, separately from hired squad members and contracts. His actual tactical stats, cavalry mount and finite kit participate in combat. His returned state persists for any withdrawal/retry instead of regenerating equipment. Completion requires a tactical victory with the commander alive. A dead commander produces an explicit terminal mission defeat; there is no silent resurrection or premature recruitment. The full San Lorenzo authored-map opening test now uses `enterSector`, issues legal tactical orders, and verifies a surviving commander contributes shots before victory.

`tests/missions-web.test.mjs` covers physical conference progression, incomplete departure/resumption, town/scene isolation, saves, temporary allied identity, rejected premature victory, and actual enemy-phase commander death. The strategic full-campaign test uses explicit outcome fixtures for its broader resource/progression coverage; it is not represented as an autonomous full tactical campaign playthrough. The separate opening test exercises actual combat actions on authored San Nicolás and San Lorenzo maps.
