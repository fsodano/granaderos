// Death holds the exact last authored collapse frame.
...
for(const appearance of Object.keys(SPRITE_APPEARANCES))for(const [aim,fire] of [['dead-idle','collapse']]){
 ...
 for(let d=0;d<8;d++){
  const bytes=await sharp(`${directory}/${entry.file}`).extract({left:(entry.framesPerDirection-1)*entry.cell,top:d*entry.cell,width:entry.cell,height:entry.cell}).png().toBuffer();
  layers.push({input:bytes,left:d*entry.cell,top:0});
  records.push({...entry.records.find(r=>r.direction===base.directions[d]&&r.frame===entry.framesPerDirection-1),frame:0,sha256:sha(bytes)});
 }
 ...
 const derived={...entry,file:`${name}.png`,framesPerDirection:1,fps:0,size:[entry.cell*8,entry.cell],records,sha256:sha(bytes),derivedFrom:{atlas:sourceName,frame:entry.framesPerDirection-1,reason:'hold final collapse frame without breathing'}};
