  ...
  try{
    ...
    switch(action.type){
      ...
      case 'battleResult':{
        ...
        const survivors=(action.survivors??[]).filter(r=>!request.garrison?.some(g=>g.id===Number(r.id))&&!action.sectorState?.units?.some(u=>u.militia&&Number(u.id)===Number(r.id)));requireThat(Array.isArray(survivors),'El parte de bajas es inválido.');
        ...
        const battleSnapshot=action.sectorState?validateSectorSnapshot(action.sectorState):null;returnGarrison(s,request,battleSnapshot);s.resources.cartridges+=returnAmmunition(request,survivors,battleSnapshot);
...
export function restoreCampaign(text){
  ...
  validatePolitics(s);s.lastError=null;return s;
}
