...
export function deploymentCost(s){const roster=rosterFor(s);return s.squad.reduce((total,id)=>total+(WEAPONS[roster.find(o=>o.id===id)?.weapon]?.capacity?10:0),0);}
  ...
  try{
    ...
    switch(action.type){
      ...
      case 'diplomacy':{
        ...
        else if(kind==='requisition'){requireThat(policyStatus(s).requisitionReady,'Las estancias necesitan catorce días para recuperarse.');s.politics??={};s.politics.requisitionAfter=s.hour+336;add(s,{treasury:200});standing(s,'gauchos',-20);standing(s,'directory',-10);s.sectors[s.location].loyalty=Math.max(0,s.sectors[s.location].loyalty-20);note(s,'La requisa abastece al ejército, pero provoca rechazo en la población.');}
