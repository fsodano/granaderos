...
export function applyPolicy(s,kind){
 ...
 if(kind==='tax'){
  ...
  s.politics.requisitionAfter=s.hour+336;s.resources.horses+=8;standing(s,'indigenous',-35);standing(s,'gauchos',-15);
  ...
  note(s,'Se toman ocho caballos de la frontera. La población denuncia la violación de sus acuerdos.');
...
export function dailyPolitics(s){
 ...
 if(s.hour%168===0){
  ...
  if(s.reputation.indigenous<0){const at=['uspallata','los_patos','mendoza'].find(id=>s.sectors[id].owner==='patriot');if(at){const lost=Math.min(5,s.resources.horses);s.resources.horses-=lost;s.sectors[at].damageUntil=Math.max(s.sectors[at].damageUntil,s.hour+72);note(s,`Una partida de frontera recupera ${lost} caballos y perturba la actividad de las estancias de Cuyo durante tres días.`);}}
 }
}
