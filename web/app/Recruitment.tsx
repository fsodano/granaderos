...
export default function Recruitment({state:s,dispatch}:Props){
 ...
 const card=(o:any)=>{
   ...
   return <article className={`officer ${active?'in-squad':''}`} key={o.id}>
    ...
    <p className="officer-bio">{o.biography}</p><button className="dossier-link" onClick={()=>setDossierId(o.id)}>Ver atributos, especialidades y equipo →</button>
 ...
 return <section className="roster-section"><div className="section-intro"><h2>Al servicio de la patria</h2><p>Incorporá oficiales y elegí hasta seis combatientes. Escuadra actual: {s.squad.length}/6.</p></div>
