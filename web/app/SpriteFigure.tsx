...
export default function SpriteFigure({unit,position,motion,pose='idle',drawSize=52,appearance='soldier'}:Props){
 ...
 const action=appearance==='soldier'&&['fire','reload','strike'].includes(pose)?pose:null;
 ...
 if(appearance==='civilian'){name=`civilian-${motion.moving?'walk':'idle'}`;if(motion.moving){col=frame;row=dir;rows=8;}}
 else if(unit.mounted){cell=256;size=drawSize*1.62;name=`cavalry-${motion.moving?'walk':'idle'}`;anchor=.8828744590;if(motion.moving){col=frame;row=dir;rows=8;}}
