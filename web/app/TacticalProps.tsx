...
export function buildPropObjects({state, revealed, project, light}:Props){
 return (state.props??[]).flatMap((prop:any)=>{
  ...
  const p=project(prop.x,prop.y);let furniture:ReactNode;
  ...
  return [{key:`prop-${prop.id}`,depth:prop.x+prop.y+.02,node:<g data-prop-id={prop.id} data-prop-type={prop.type} transform={`translate(${p.x} ${p.y})`} pointerEvents="none" style={{filter:`brightness(${light(prop.x,prop.y)})`}}><ellipse cx="6" cy="4" rx="23" ry="9" fill="#1d1e14" opacity=".3"/>{furniture}</g>}];
 });
}
